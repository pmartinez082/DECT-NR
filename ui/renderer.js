...
window.addEventListener('DOMContentLoaded', () => {
  ...
  document.getElementById('pdf-select').addEventListener('change', (e) => {
    ...
    ipcRenderer.invoke('get-data-dir').then((dataDir) => {
      embed.src = `file://${dataDir}/output/graphs/${selected}.pdf?cb=${new Date().getTime()}`;
    }).catch(() => {
  ...
  document.getElementById('start-sweep').addEventListener('click', () => {
    ...
    ipcRenderer.send('start-sweep', { snrRanges, channelType, serverSerial, clientSerial, snrStep });
  });
...
ipcRenderer.on('refresh-graph', () => {
  ...
  ipcRenderer.invoke('get-data-dir').then((dataDir) => {
    embed.src = `file://${dataDir}/output/graphs/TDL-B.pdf?cb=${new Date().getTime()}`;
  }).catch(() => {
