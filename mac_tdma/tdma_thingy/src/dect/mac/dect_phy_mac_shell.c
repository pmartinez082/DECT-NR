...
static const char dect_phy_mac_auto_client_cmd_usage_str[] =
	...
	"Options:\n"
	"  -c, --channel <channel>,            Channel to scan for beacons. Default: 1677.\n"
	"  -s, --scan_time <seconds>,          Scan duration in seconds. Default: 4.\n"
	"  -p, --tx_pwr <integer>,             TX power for associate and RACH TX. Default: 0.\n"
	"  -m, --tx_mcs <integer>,             MCS for associate and RACH TX. Default: 0.\n"
	"  -d, --data <string>,                Data payload for RACH TX. Default: auto.\n"
	"  -x, --msg_tx_id <id>,               Message TX ID in payload. Default: 0.\n"
	"  -i, --interval <seconds>,           RACH TX interval. Default: 2.\n"
	"  -q, --tdma_tx_iteration_multiplier <integer>, 40 ms spacing if set to 4. Default: 4.\n"
	"  -r, --tdma_tx_iteration_count <integer>,      Number of iterations. Default: 50.\n"
	"  -w, --wait_after_assoc <seconds>,    Wait after association before starting TX. Default: 20.\n"
	"  -h, --help,                         Show this help.\n";

enum {
	DECT_PHY_MAC_AUTO_CLIENT_OPT_WAIT_AFTER_ASSOC = 1001,
};

static struct option long_options_auto_client[] = {
	{"channel", required_argument, 0, 'c'},
	{"scan_time", required_argument, 0, 's'},
	...
	{"wait_after_assoc", required_argument, 0, DECT_PHY_MAC_AUTO_CLIENT_OPT_WAIT_AFTER_ASSOC},
	{"help", no_argument, 0, 'h'},
	{0, 0, 0, 0}
};

static int dect_phy_mac_auto_client_cmd(const struct shell *shell, size_t argc, char **argv)
{
	struct dect_phy_mac_beacon_scan_params scan_params = {
		.clear_nbr_cache_before_scan = true,
		.expected_rssi_level = 0,
		.duration_secs = 4,
		.channel = 1677,
		.suspend_scheduler = false,
	...
	struct dect_phy_settings *current_settings = dect_common_settings_ref_get();
	int wait_after_assoc = 20;
	int ret = 0;
	int long_index = 0;
	int opt;

	memset(&rach_params, 0, sizeof(rach_params));
	...
	strcpy(rach_params.tx_data_str, "auto");

	optreset = 1;
	optind = 1;
	while ((opt = getopt_long(argc, argv, "c:s:p:m:d:x:i:q:r:w:h", long_options_auto_client,
				  &long_index)) != -1) {
		switch (opt) {
		case 'c': {
			scan_params.channel = atoi(optarg);
			break;
		}
		case 's': {
			scan_params.duration_secs = atoi(optarg);
			if (scan_params.duration_secs <= 0) {
				desh_error("Scan time must be positive.");
				return -EINVAL;
			}
			break;
		}
		case 'p': {
			assoc_params.tx_power_dbm = atoi(optarg);
			rach_params.tx_power_dbm = assoc_params.tx_power_dbm;
			break;
		}
		case 'm': {
			assoc_params.mcs = atoi(optarg);
			rach_params.mcs = assoc_params.mcs;
			break;
		}
		case 'd': {
			strcpy(rach_params.tx_data_str, optarg);
			break;
		}
		case 'x': {
			rach_params.tx_id = (uint16_t)shell_strtoul(optarg, 10, &ret);
			if (ret) {
		...
		case 'i': {
			rach_params.interval_secs = atoi(optarg);
			if (rach_params.interval_secs < 0) {
		...
		case 'q': {
			...
			rach_params.tdma_tx_iteration_multiplier = (uint8_t)value;
			break;
		}
		case 'r': {
			unsigned long value = shell_strtoul(optarg, 10, &ret);
			...
			rach_params.tdma_tx_iteration_count = (uint8_t)value;
			break;
		}
		case 'w':
		case DECT_PHY_MAC_AUTO_CLIENT_OPT_WAIT_AFTER_ASSOC: {
			wait_after_assoc = atoi(optarg);
			if (wait_after_assoc < 0) {
				desh_error("Wait time must be positive.");
				return -EINVAL;
			}
			break;
		}
	...
	desh_print("Starting auto client: scan channel %u for %d seconds.",
		scan_params.channel, scan_params.duration_secs);
	ret = dect_phy_mac_ctrl_beacon_scan_start(&scan_params);
	if (ret) {
		desh_error("Cannot start beacon scan, err %d", ret);
		return ret;
	}

	k_sleep(K_SECONDS(scan_params.duration_secs + 1));
	dect_phy_ctrl_rx_stop();

	{
		struct dect_phy_mac_nbr_info_list_item *neighbor =
			dect_phy_mac_nbr_info_get_first(scan_params.channel);
		if (!neighbor) {
			desh_error("No beacon found on channel %u", scan_params.channel);
			return -ENOENT;
		}

		desh_print("Found beacon target long RD ID %u, short RD ID %u",
			neighbor->long_rd_id, neighbor->short_rd_id);

		assoc_params.target_long_rd_id = neighbor->long_rd_id;
		ret = dect_phy_mac_ctrl_associate(&assoc_params);
		if (ret) {
			desh_error("Cannot start association, err %d", ret);
			return ret;
		}
		desh_print("Association request sent to %u. Waiting %d seconds before TX.",
			assoc_params.target_long_rd_id, wait_after_assoc);
		k_sleep(K_SECONDS(wait_after_assoc));

		rach_params.target_long_rd_id = neighbor->long_rd_id;
		ret = dect_phy_mac_ctrl_rach_tx_start(&rach_params);
		if (ret) {
			desh_error("Cannot start RACH TX, err %d", ret);
			return ret;
		}
		desh_print("Auto RACH TX started to master %u.", rach_params.target_long_rd_id);
	}

	return 0;

show_usage:
	desh_print_no_format(dect_phy_mac_auto_client_cmd_usage_str);
	return 0;
}
...
SHELL_STATIC_SUBCMD_SET_CREATE(
	...
	SHELL_CMD_ARG(auto_client, NULL, "Usage options: dect mac auto_client -h",
		      dect_phy_mac_auto_client_cmd, 1, 20),
