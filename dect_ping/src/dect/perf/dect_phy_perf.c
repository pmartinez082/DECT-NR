			       ...
			       enum dect_phy_perf_server_restart_mode restart_mode)
{
	...
	if (params->role == DECT_PHY_COMMON_ROLE_CLIENT) {
		...
		//desh_print("Starting server RX: channel %d, exp_rssi_level %d, duration %d secs.", params->channel, params->expected_rx_rssi_level, params->duration_secs);
		...
		if (DECT_PHY_PERF_TX_HANDLE_IN_RANGE(mdm_completed_params->handle)) {
			...
			if (secs_left) {
				...
				if (secs_left == DECT_PHY_PERF_DURATION_INFINITE) {
					//desh_print("Server RX will be continued after result TX is done.");
				} else {
					//desh_print("Server RX will be continued (%d secs) after result TX is done.", secs_left);
				}
...
static void dect_phy_perf_thread_fn(void)
{
	...
	while (true) {
		...
		case DECT_PHY_PERF_EVENT_RX_PCC_CRC_ERROR: {
			...
			desh_print("PCC_ERR,%u.%02u,%d,%d", cmd_params->pdc_number/100, cmd_params->pdc_number%100, perf_data.rx_metrics.rx_latest_mcs, params->crc_failure.snr);
			break;
		}
		case DECT_PHY_PERF_EVENT_RX_PDC_CRC_ERROR: {
			...
			desh_print("PDC_ERR,%u.%02u,%d,%d", cmd_params->pdc_number/100, cmd_params->pdc_number%100, perf_data.rx_metrics.rx_latest_mcs, params->crc_failure.snr);
			break;
		}
...
int dect_phy_perf_cmd_handle(struct dect_phy_perf_params *params)
{
	...
	if (params->pdc_number == 0) {
		params->pdc_number = 10000; /* 100.00 */
	}
...
static int dect_phy_perf_rx_pdc_data_handle(struct dect_phy_data_rcv_common_params *params)
{
	...
	if (pdu.header.message_type == DECT_MAC_MESSAGE_TYPE_PERF_TX_DATA) {
		...
		desh_print("pdc,%u.%02u,%d,%d",perf_data.cmd_params.pdc_number/100,perf_data.cmd_params.pdc_number%100,perf_data.rx_metrics.rx_latest_mcs, params->snr);
